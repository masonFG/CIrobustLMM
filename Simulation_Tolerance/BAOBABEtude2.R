...
bG = -0.05 #beta groupe
bT = 0.04 #beta time
#bGxT = 0.125 #beta interaction groupe fois time
...
varRES_n = 0.1#variance erreurs normale
varRES_e = 0.001#variance erreurs extremes 
moyRES_e = typeExt*4*sqrt(varRES_n)#moyenne des erreurs etremes
matran_n = matrix(c(0.05,0.01,0.01,0.02),2,2)#matrice variance covariance normale
matran_e = matrix(c(0.0005,0.0001,0.0001,0.0002),2,2)#matrice variance covariance extreme
moyran_Ie = c(typeExt*4*sqrt(0.05),0)#vecteur moyennes des random intercept
moyran_Pe = c(0,typeExt*4*sqrt(0.02))#vecteur moyennes des random pente
...
bdd$y= b0 + bG * bdd$group + bT * bdd$time + bGxT * bdd$group * bdd$time + bdd$u0 + bdd$u1 * bdd$time + bdd$r
...
if(estimator == "tML"){
model.tML = heavyLme(y ~ 1 + group*time, random = ~ 1 + time, groups = ~ id, data = Dataset) 

if(bootstrap == "wild"){
  if(test == TRUE){
model.tML0 = heavyLme(y ~ 1 + group + time, random = ~ 1 + time, groups = ~ id, data = Dataset) 
