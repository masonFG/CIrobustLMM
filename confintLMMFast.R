...
confint.LMM <- function(model, Data, id, Time, method, B, level){
  ...
  if(class(model)[1]=="varComprob.S"){
    ...
    if(method == "wild"){
      source("CIfunction_wildREMLS.R")
      result = wild_REMLvarComp(model = model, id = id, Time = Time, B = B, level = level)
      return(result)
    }
    if(method == "Wald"){
      summ = summary(model)
  ...
  if(class(model)[1]=="varComprob.compositeTau"){
    ...
    if(method == "wild"){
      source("CIfunction_wildREMLcTAU.R")
      result = wild_REMLvarComp(model = model, id = id, Time = Time, B = B, level = level)
      return(result)
    }
    if(method == "Wald"){
  ...
  if(class(model)[1]=="heavyLme"){
    ...
    if(method == "wild"){
      source("CIfunction_wildREMLheavyLme.R")
