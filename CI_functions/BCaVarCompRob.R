...
confint_BCaVarComprob <- function(B, model, data, clusterid, coefs, res.or.coef, p, confint.Zboundaries){
  B_alt <- rep(B,p)
...
BCabootvarCompRob <- function(model, data, clusterid, methodCI, B=B, confint.level=confint.level, BCa = BCa){
  ...
  if(BCa == T){
  ci_BCa <- confint_BCaVarComprob(B, model, data, clusterid, coefs, res.or.coef, p, confint.Zboundaries)
